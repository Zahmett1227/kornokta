...
final class AppEnvironment: ObservableObject {
    ...
    ///
    /// Built per use for the same reason as the two above: a button on a screen
    /// the user reaches rarely, so resolving here keeps an edited URL or token
    /// current without joining `backendChanged()`'s rebuild list.
