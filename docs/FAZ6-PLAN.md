...
| **B3** | Gerçek sayfalarla prompt kalite döngüsü (işaret odağı + el yazısı okuma) | 🟡 **Başladı.** Prompt v2.1 (işaret-öncelik hiyerarşisi, el yazısı=niyet, yüksek-getirili TUS çerçeveleme, cloze/ayırt-etme/mekanizma kalite kuralları, disiplinli zenginleştirme + lowConfidence). `OPENAI_REASONING_EFFORT` varsayılanı `low`→`medium`. Backend 425 test yeşil. Asıl iş: gerçek sayfalarla iterasyon (cihaz). |
