...
| Çıkmış soru bankası (ADR-012, 2026-09-25) | 🟡 **Faz 0 + Faz A1 tamam** `cikmis-soru-bankasi` dalında (uygulama tarafı Faz A2'de). **Faz A1 (banka hattı):** A1–A9 gerçek klasörde koştu, **V1–V10 geçti** (V9: Tuğba Çağlar, 30 soru; incelemenin bulduğu tablo-şık hatası bankanın tamamında düzeltildi). Paket `tools/exam_bank/out/CizgiSoruBankasi/` (gitignore'lu, sürüm 2026-09-25.2): 8.410 soru (ok 6.985, anahtarsız 1.357, iptal 55, değiştirilmiş 12), 85 kağıt, 82 PDF, 163 MB. Model $0,65. Ölçümün düzelttikleri planın "Faz A1 sonucu" bölümünde (2011/1 = Temel-1+2; Klinik 6 blok; derlemenin 2024/1 Klinik numara kayması → 22 soru dışarıda). Telefon bu paketi henüz içe aktaramıyor — o Faz A2. Faz 0: Vizyon panelinin ("kitap yanımda olmadan çalışma") çekirdek önerisi, sahibinin 86 çıkmış soru PDF'ine göre planlandı ve onaylandı: [`docs/PLAN-cikmis-soru-bankasi.md`](docs/PLAN-cikmis-soru-bankasi.md). Faz 0'da: kart yüzü `Card` yerine `StudyFaceContent` değeri çiziyor (Tekrar simülatörde piksel piksel aynı, Egzersiz'de 0,002 piksellik gözle görülmez kayma), `FesScore.record` tek canlı FES yazarı, `tools/exam_bank/` kaynak kaydı (82 dosya, 85 kağıt, sha256 sabitli) + `--dry-run`. **Sıradaki:** Faz A2 (uygulama çekirdeği) |
...
**Kod tarafında sıradaki:** Çıkmış soru bankası Faz A2 — uygulama çekirdeği
(`ExamBankDocument` + `exam_bank.schema.json` kilidi, SwiftData modelleri, paket içe
aktarma, Pratik; `docs/PLAN-cikmis-soru-bankasi.md` §9.3). Paket hazır
(`tools/exam_bank/out/CizgiSoruBankasi/`, sürüm 2026-09-25.2). Sahibinin işi: eksik
