...
| `api/_coverage.ts` | `POST /api/coverage` — kartlaşmamış işaretlerin Gemini ile bağımsız denetimi (Katman B) |
| `providers/coverage.ts` | Kapsama sözleşmesi (şema v2.3, Katman A): `marks[]` ↔ kart `markId` farkını deterministik çıkarır — *kartsız işaret* ve *işaretsiz kart* |
